import 'package:flutter/material.dart';
import 'package:just_audio/just_audio.dart';

class HomeScreen extends StatefulWidget {
  const HomeScreen({super.key});

  @override
  State<HomeScreen> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<HomeScreen> {
  @override
  Widget build(BuildContext context) {
    var images = [

      GestureDetector(
        child: Image.network( 'https://i.ebayimg.com/images/g/rQ8AAOSws3phwv80/s-l1600.png'),
        onTap: () async {
          print(':heart_eyes:');
          final player = AudioPlayer();                   // Create a player
          await player.setUrl(           // Load a URL
         'https://samplelib.com/lib/preview/mp3/sample-3s.mp3');                 // Schemes: (https: | file: | asset: )
          player.play();    
        }),
      GestureDetector(
        child: Image.network('https://i.imgflip.com/60xf1m.jpg'),
        onTap: () => print('spongebob')),
      GestureDetector(
        child: Image.network('https://i0.wp.com/www.comicbookrevolution.com/wp-content/uploads/2023/09/Nightwing-106-Dan-Mora-Banner.jpg?fit=1540%2C705&ssl=1'),
        onTap: () => print('Naomi will edit this')),
      GestureDetector(
        child: Image.network('https://m.media-amazon.com/images/I/7122KMkSuVL.__AC_SY300_SX300_QL70_FMwebp_.jpg'),
        onTap: () async {
          print('USA!!!');
          final player = AudioPlayer();                   // Create a player
          await player.setUrl(           // Load a URL
          '');   
        }),
  
      GestureDetector(
        child: Image.network('data:image/jpeg;base64,'),
        onTap: () => print('Im a Banana!')),
      GestureDetector(
        child: Image.network('https://i1.sndcdn.com/artworks-TXWqD6NbJRXPhUns-8fYa5g-t500x500.jpg'),
        onTap: () => print('Annoying orange is a nightmare')),
      GestureDetector(
        child: Image.network('data:image/jpeg;base64,'),
        onTap: () => print('I used to be this')),
      GestureDetector(
        child: Image.network('https://i1.sndcdn.com/artworks-HdxXE6BxJ65FHooi-rtiaPw-t500x500.jpg'),
        onTap: () => print('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA')),
      GestureDetector(
        child: Image.network('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTMo2Csw_a32vmMA-xruyOKwkED6E4VF4ms0A&usqp=CAU'),
        onTap: () => print('OOOOOOOOOOOOOOOOOOOOOOOOA')),
      GestureDetector(
        child: Image.network('https://64.media.tumblr.com/c971848e88b848404fbaa7ffe165139d/520514081478ab09-11/s1280x1920/46d81fef21acd1fca90a350138cec99ec5a20697.png'),
        onTap: () => print('Is your jaw ok?, Ayden DID NOT DRAW THIS')),
      GestureDetector(
        child: Image.network('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT6HtdLeEa4WlyO8DwTB19MPS9LekYsM98OvouDYEpVD9AxMUX5BR_t1dNAj-hcooQO9Dg&usqp=CAU'),
        onTap: () => print('sad violin theme')),
    ];

    return GridView.count(crossAxisCount: 2,
    children: images
    );
  }
}
